...
public class MyfileController {
    ...
    public WebPageVo uploadFiles(@RequestParam("files") List<MultipartFile> files, HttpSession session) {
        ...
        int idCounts = this.myfileService.findIdCounts();
        ...
        for (MultipartFile file : files) {
            ...
            //设置文件上传路径
            String path = "D:\\storage\\yunfile\\files\\";
            //获取文件大小(单位kb)
    ...
    public String findAllFiles(HttpSession session, Model model) {
        ...
        List<Myfile> allFilesList = this.myfileService.findAllFiles();
    ...
    public String findAllRecycles(HttpSession session,Model model){
        List<Myfile> allRecycleList = this.myfileService.findAllRecycles();
    ...
    public void deleteAllFiles(Integer ids[]){
        for (Integer id : ids) {
            ...
            //创建删除文件对象
            File file = new File("D:\\storage\\yunfile\\files\\",delfile.getNewfilename());
            //进行删除
